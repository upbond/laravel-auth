...
To protect APIs using an access token generated by Auth, there is an `auth0` API guard provided ([Laravel documentation on guards](https://laravel.com/docs/7.x/authentication#adding-custom-guards)). To use this guard, add it to `config/auth.php` with the driver `auth0`:
```
'guards' => [
    ...
    'auth0' => [
